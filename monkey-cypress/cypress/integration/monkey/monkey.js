    ...
    cy.window().then((win)=>{
        ...
        if(!!element){
            ...
            if(!!element.id){ //boolean that indicates if the element has a non-empty id
                cy.get(`#${element.id}`).scrollIntoView({force:true})
    ...
    it(`visits ${appName} and survives monkeys`, function() {
        ...
        if(pcg === 100){
            ...
            for(let i = 0; i < events + 5; i++){
                ...
                cy.location().then((newLoc) => {
                    // Compare new location with base url

                    if (!newLoc.href.startsWith(url)) {
                        cy.task("logCommand", { funtype: 'Navigation to a page outside the current domain was prevented.', info: {}})
                        cy.go('back');
                    }
                });
            }
