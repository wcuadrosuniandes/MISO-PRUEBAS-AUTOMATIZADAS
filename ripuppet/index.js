...
async function scrapLinks(page) {
  const stories = await page.evaluate(() => {
    ...
    const links = anchors.map(
      (anchor) => new URL(anchor.href, document.location.href).href
    );
    const webLinks = links.filter(
      (link) =>
        link.toString().startsWith("http://localhost") &&
        !link.toString().includes("#/editor/post/")
    );
...
async function getTextInputs(page, elementList) {
  ...
  for (let i = 0; i < textInputs.length; i++) {
    let disabled = await page.evaluate((inp) => {
      return (
        typeof inp.getAttribute("disabled") === "string" ||
        inp.getAttribute("aria-disabled") === "true" ||
        inp.getAttribute("type") == "file"
      );
    }, textInputs[i]);
    if (!disabled) {
      input = {
  ...
  for (let i = 0; i < buttons.length; i++) {
    let disabled = await page.evaluate((btn) => {
      return (
        ...
        btn.getAttribute("id") == "ember11" ||
        btn.getAttribute("class") == "gh-nav-btn-search"
      );
    }, buttons[i]);
    if (!disabled) {
      button = {
